...
contract Source {
    ...
    function callFuncOneParam() external {
        doCall(abi.encodeWithSelector(dest.funcOneParam.selector, 17));
    }
    function callFuncTwoParams() external {
        doCall(abi.encodeWithSelector(dest.funcTwoParams.selector, 17, 23));
    }

    function doCall(bytes memory functionCall) private {
        bytes memory authVals = abi.encodePacked(e1, e2, e3);
        bytes memory functionCallWithAuth = bytes.concat(functionCall, authVals);
